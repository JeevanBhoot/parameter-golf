...
INT8_RTN_QMAX = 15
INT8_AUTO_HIGH_QMAX = 31
INT8_AUTO_HIGH_PRECISION_ROLE_COUNT = 1
...
def tensor_role_name(name: str) -> str:
    parts = name.split(".")
...
def quantize_float_array(arr: mx.array, qmax: int = INT8_RTN_QMAX) -> tuple[np.ndarray, np.ndarray, str]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        ...
        return np.ascontiguousarray(q), np.ascontiguousarray(scale.astype(INT8_PER_ROW_SCALE_DTYPE, copy=False)), "per_row"
    ...
    return np.ascontiguousarray(q), scale, "per_tensor"
...
def dequantize_quantized_array(
    ...
    scale: np.ndarray,
    scheme: str,
    group_size: int = INT8_PER_ROW_GROUP_SIZE,
) -> np.ndarray:
    if scheme == "per_row_group":
        ...
        return out_arr
    if scheme == "per_row" or scale.ndim > 0:
        return q_np.astype(np.float32) * scale.reshape((q_np.shape[0],) + (1,) * (q_np.ndim - 1))
    return q_np.astype(np.float32) * float(scale)


def relative_quantization_mse(
    f32: np.ndarray,
    q_np: np.ndarray,
    scale: np.ndarray,
    scheme: str,
) -> float:
    recon = dequantize_quantized_array(q_np, scale.astype(np.float32, copy=False), scheme)
    denom = float(np.mean(np.square(f32), dtype=np.float64)) + 1e-12
    return float(np.mean(np.square(recon - f32), dtype=np.float64) / denom)


def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, object]]:
    ...
    quantizable: dict[str, dict[str, object]] = {}
    role_error_sums: dict[str, float] = {}
    role_error_counts: dict[str, int] = {}
    stats = dict.fromkeys(
    ...
    for name, arr in flat_state.items():
        ...
        q_low, s_low, scheme = quantize_float_array(arr, qmax=INT8_RTN_QMAX)
        role = tensor_role_name(name)
        rel_mse = relative_quantization_mse(_np_float32(arr), q_low, s_low.astype(np.float32, copy=False), scheme)
        role_error_sums[role] = role_error_sums.get(role, 0.0) + rel_mse
        role_error_counts[role] = role_error_counts.get(role, 0) + 1
        quantizable[name] = {
            "arr": arr,
            "role": role,
            "q_low": q_low,
    ...
    role_scores = {
        role: role_error_sums[role] / max(role_error_counts[role], 1)
        for role in role_error_sums
    }
    selected_roles = {
        role
        for role, _ in sorted(role_scores.items(), key=lambda item: (-item[1], item[0]))[:INT8_AUTO_HIGH_PRECISION_ROLE_COUNT]
    }

    for name, info in quantizable.items():
        role = info["role"]
        qmax = INT8_AUTO_HIGH_QMAX if role in selected_roles else INT8_RTN_QMAX
        if qmax == INT8_RTN_QMAX:
            q = info["q_low"]
            s = info["s_low"]
            scheme = info["scheme"]
        else:
            q, s, scheme = quantize_float_array(info["arr"], qmax=qmax)
        if scheme == "per_row_group":
            qmeta[name] = {"scheme": scheme, "axis": 0, "group_size": INT8_PER_ROW_GROUP_SIZE, "qmax": qmax}
        elif scheme == "per_row":
            qmeta[name] = {"scheme": scheme, "axis": 0, "qmax": qmax}
        else:
            qmeta[name] = {"qmax": qmax}
        quantized[name] = q
        scales[name] = s
        dtypes[name] = str(info["arr"].dtype).split(".")[-1]
        stats["int8_payload_bytes"] += int(q.nbytes + s.nbytes)

    stats["auto_high_precision_roles"] = ",".join(sorted(selected_roles))
    stats["auto_high_precision_role_count"] = len(selected_roles)
    obj: dict[str, object] = {
        "__quant_format__": "int5_group64_top1roleint6_tiedemb_fp16_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
    ...
    if selected_roles:
        obj["auto_high_precision_roles"] = sorted(selected_roles)
    return obj, stats
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q in quant_obj["quantized"].items():
        ...
        scheme = str(qmeta.get(name, {}).get("scheme", "per_tensor"))
        group_size = int(qmeta.get(name, {}).get("group_size", INT8_PER_ROW_GROUP_SIZE))
        out_arr = dequantize_quantized_array(q_np, scale, scheme, group_size=group_size)
...
def main() -> None:
    ...
    if quant_stats.get("auto_high_precision_roles"):
        log(
            f"auto_high_precision_roles:{quant_stats['auto_high_precision_roles']} "
            f"count:{quant_stats['auto_high_precision_role_count']}"
        )
