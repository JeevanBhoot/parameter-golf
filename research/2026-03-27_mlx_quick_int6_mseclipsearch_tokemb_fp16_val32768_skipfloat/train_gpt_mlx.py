...
INT8_RTN_QMAX = 31
INT8_MSE_CLIPSEARCH_QUANTILES = tuple(
    sorted(
        {
            1.0,
            INT8_CLIP_Q,
...
def _mse_search_quantize_per_row(f32: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    if not f32.size:
        return (
            np.empty(f32.shape, dtype=np.int8),
            np.empty((f32.shape[0],), dtype=INT8_PER_ROW_SCALE_DTYPE),
        )

    candidate_qs = np.asarray(INT8_MSE_CLIPSEARCH_QUANTILES, dtype=np.float32)
    abs_f32 = np.abs(f32)
    clip_abs = np.quantile(abs_f32, candidate_qs, axis=1).transpose(1, 0).astype(np.float32, copy=False)
    ...
    q_candidates = np.clip(np.round(clipped / scale[:, None, :]), -INT8_RTN_QMAX, INT8_RTN_QMAX).astype(
        np.int8,
        copy=False,
    )
    dequantized = q_candidates.astype(np.float32) * scale[:, None, :]
    mse = np.mean(np.square(dequantized - f32_expanded), axis=1)
    best_idx = np.argmin(mse, axis=1)
...
def _mse_search_quantize_per_tensor(f32: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    if not f32.size:
        return np.ascontiguousarray(np.empty(f32.shape, dtype=np.int8)), np.array(1.0, dtype=np.float32)

    candidate_qs = np.asarray(INT8_MSE_CLIPSEARCH_QUANTILES, dtype=np.float32)
    flat = f32.reshape(-1)
    clip_abs = np.quantile(np.abs(flat), candidate_qs).astype(np.float32, copy=False)
    scale = np.maximum(clip_abs / float(INT8_RTN_QMAX), 1.0 / float(INT8_RTN_QMAX)).astype(np.float32, copy=False)
    clipped = np.clip(flat[:, None], -clip_abs[None, :], clip_abs[None, :])
    q_candidates = np.clip(np.round(clipped / scale[None, :]), -INT8_RTN_QMAX, INT8_RTN_QMAX).astype(
        np.int8,
        copy=False,
    )
    dequantized = q_candidates.astype(np.float32) * scale[None, :]
    mse = np.mean(np.square(dequantized - flat[:, None]), axis=0)
    best_idx = int(np.argmin(mse))
    q_best = q_candidates[:, best_idx].reshape(f32.shape)
    scale_best = np.array(scale[best_idx], dtype=np.float32)
    return np.ascontiguousarray(q_best), scale_best


def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        # Search a small set of clip quantiles under the INT6 dynamic range and
        # keep the lowest-MSE candidate per row.
        return _mse_search_quantize_per_row(f32)

    # Vectors / scalars use the same clip search, but with one scale per tensor.
    return _mse_search_quantize_per_tensor(f32)
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int6_mseclipsearch_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
