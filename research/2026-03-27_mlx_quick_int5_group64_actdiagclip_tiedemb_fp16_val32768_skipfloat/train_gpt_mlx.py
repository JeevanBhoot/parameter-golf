...
class Hyperparameters:
    ...
    calibration_seqs: int = int(os.environ.get("CALIBRATION_SEQS", 8))
...
class ActivationSecondMomentCollector:
    ...
    def record(self, key: str, x: mx.array) -> None:
        flat = np.array(x.astype(mx.float32), dtype=np.float32, copy=False).reshape(-1, x.shape[-1])
        if flat.size == 0:
            return
        sumsq = np.sum(flat * flat, axis=0, dtype=np.float64)
        if key in self.sumsq:
            self.sumsq[key] = self.sumsq[key] + sumsq
        else:
            self.sumsq[key] = sumsq
        self.counts[key] = self.counts.get(key, 0) + int(flat.shape[0])

    def means(self) -> dict[str, np.ndarray]:
        return {
            key: np.ascontiguousarray((sumsq / max(self.counts[key], 1)).astype(np.float32, copy=False))
            for key, sumsq in self.sumsq.items()
        }


ACTIVATION_SECOND_MOMENT_COLLECTOR: ActivationSecondMomentCollector | None = None

class CastedLinear(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        super().__init__()
        self.weight = nn.Linear(in_dim, out_dim, bias=False).weight.astype(mx.float32)
        self.stat_key: str | None = None

    def __call__(self, x: mx.array) -> mx.array:
        if self.stat_key is not None and ACTIVATION_SECOND_MOMENT_COLLECTOR is not None:
            ACTIVATION_SECOND_MOMENT_COLLECTOR.record(self.stat_key, x)
...
def named_casted_linear_modules(model: GPT) -> list[tuple[str, CastedLinear]]:
    named_modules: list[tuple[str, CastedLinear]] = []
    for block_idx, block in enumerate(model.blocks):
        prefix = f"blocks.{block_idx}"
        named_modules.extend(
            [
                (f"{prefix}.attn.c_q.weight", block.attn.c_q),
                (f"{prefix}.attn.c_k.weight", block.attn.c_k),
                (f"{prefix}.attn.c_v.weight", block.attn.c_v),
                (f"{prefix}.attn.proj.weight", block.attn.proj),
                (f"{prefix}.mlp.fc.weight", block.mlp.fc),
                (f"{prefix}.mlp.proj.weight", block.mlp.proj),
            ]
        )
    return named_modules


def collect_activation_sq_means(
    model: GPT,
    args: Hyperparameters,
    log_fn: Callable[[str], None] | None = None,
) -> dict[str, np.ndarray]:
    if args.calibration_seqs <= 0:
        return {}

    calib_loader = TokenLoader(args.train_files)
    calib_batch_tokens = args.calibration_seqs * args.train_seq_len
    calib_x, _ = calib_loader.next_batch(calib_batch_tokens, args.train_seq_len)
    if log_fn is not None:
        log_fn(
            f"activation_calibration:dataset=train_prefix "
            f"seqs:{args.calibration_seqs} tokens:{int(calib_x.size)}"
        )

    for key, module in named_casted_linear_modules(model):
        module.stat_key = key

    global ACTIVATION_SECOND_MOMENT_COLLECTOR
    ACTIVATION_SECOND_MOMENT_COLLECTOR = ActivationSecondMomentCollector()
    _ = model(calib_x)
    mx.synchronize()
    collector = ACTIVATION_SECOND_MOMENT_COLLECTOR
    ACTIVATION_SECOND_MOMENT_COLLECTOR = None

    act_sq_means = {} if collector is None else collector.means()
    if log_fn is not None:
        log_fn(f"activation_calibration:collected_tensors:{len(act_sq_means)}")
    return act_sq_means
...
INT8_ACT_DIAG_CLIPSEARCH_QUANTILES = tuple(
    sorted({1.0, INT8_CLIP_Q, 0.99999, 0.9999, 0.999, 0.995, 0.99, 0.98, 0.95})
)
INT8_RTN_QMAX = 15
INT8_PER_ROW_GROUP_SIZE = 64
...
def _rtn_quantize_per_row_like(f32: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    rows = f32.shape[0]
    clip_abs = np.quantile(np.abs(f32), INT8_CLIP_Q, axis=1) if f32.size else np.empty((rows,), dtype=np.float32)
    clipped = np.clip(f32, -clip_abs[:, None], clip_abs[:, None])
    scale = np.maximum(clip_abs / float(INT8_RTN_QMAX), 1.0 / float(INT8_RTN_QMAX)).astype(np.float32, copy=False)
    q = np.clip(np.round(clipped / scale[:, None]), -INT8_RTN_QMAX, INT8_RTN_QMAX).astype(np.int8, copy=False)
    return np.ascontiguousarray(q), np.ascontiguousarray(scale.astype(INT8_PER_ROW_SCALE_DTYPE, copy=False))


def _act_diag_clipsearch_quantize_per_row_like(
    f32: np.ndarray,
    input_sq_mean: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    rows = f32.shape[0]
    if not f32.size:
        return (
            np.empty(f32.shape, dtype=np.int8),
            np.empty((rows,), dtype=INT8_PER_ROW_SCALE_DTYPE),
        )

    weights = np.asarray(input_sq_mean, dtype=np.float32).reshape(-1)
    if weights.size != f32.shape[1] or not np.any(weights > 0.0):
        weights = np.ones((f32.shape[1],), dtype=np.float32)
    candidate_qs = np.asarray(INT8_ACT_DIAG_CLIPSEARCH_QUANTILES, dtype=np.float32)
    clip_abs = np.quantile(np.abs(f32), candidate_qs, axis=1).transpose(1, 0).astype(np.float32, copy=False)
    scale = np.maximum(clip_abs / float(INT8_RTN_QMAX), 1.0 / float(INT8_RTN_QMAX)).astype(np.float32, copy=False)
    f32_expanded = f32[:, :, None]
    clipped = np.clip(f32_expanded, -clip_abs[:, None, :], clip_abs[:, None, :])
    q_candidates = np.clip(np.round(clipped / scale[:, None, :]), -INT8_RTN_QMAX, INT8_RTN_QMAX).astype(np.int8, copy=False)
    dequantized = q_candidates.astype(np.float32) * scale[:, None, :]
    weighted_err = np.sum(np.square(dequantized - f32_expanded) * weights[None, :, None], axis=1)
    best_idx = np.argmin(weighted_err, axis=1)
    q_best = np.take_along_axis(q_candidates, best_idx[:, None, None], axis=2).squeeze(2)
    scale_best = np.take_along_axis(scale, best_idx[:, None], axis=1).squeeze(1)
    return (
        np.ascontiguousarray(q_best),
        np.ascontiguousarray(scale_best.astype(INT8_PER_ROW_SCALE_DTYPE, copy=False)),
    )


def quantize_float_array(
    arr: mx.array,
    input_sq_mean: np.ndarray | None = None,
) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        # Matrices use per-row group scales so one local outlier chunk does not
        # set the scale for an entire row at low bitwidth. When deterministic
        # calibration statistics are available, choose the clip level that
        # minimizes activation-weighted reconstruction error.
        rows, cols = f32.shape
        use_act_diag = input_sq_mean is not None and np.asarray(input_sq_mean).reshape(-1).size == cols
        if cols > INT8_PER_ROW_GROUP_SIZE:
            ...
            for group_idx in range(num_groups):
                ...
                chunk = f32[:, start:end]
                if use_act_diag:
                    group_q, group_scale = _act_diag_clipsearch_quantize_per_row_like(
                        chunk,
                        np.asarray(input_sq_mean, dtype=np.float32)[start:end],
                    )
                else:
                    group_q, group_scale = _rtn_quantize_per_row_like(chunk)
                q[:, start:end] = group_q
                scale[:, group_idx] = np.asarray(group_scale, dtype=np.float32)
            return np.ascontiguousarray(q), np.ascontiguousarray(scale.astype(INT8_PER_ROW_SCALE_DTYPE, copy=False))

        if use_act_diag:
            return _act_diag_clipsearch_quantize_per_row_like(f32, np.asarray(input_sq_mean, dtype=np.float32))
        return _rtn_quantize_per_row_like(f32)

    # Vectors / scalars use a simpler per-tensor scale.
    clip_abs = float(np.quantile(np.abs(f32).reshape(-1), INT8_CLIP_Q)) if f32.size else 0.0
    scale = np.array(clip_abs / float(INT8_RTN_QMAX) if clip_abs > 0.0 else 1.0, dtype=np.float32)
    q = np.clip(
...
def quantize_state_dict_int8(
    ...
    activation_sq_means: dict[str, np.ndarray] | None = None,
    ...
    for name, arr in flat_state.items():
        ...
        if int(arr.size) <= INT8_KEEP_FLOAT_MAX_NUMEL or name == "tok_emb.weight":
        ...
        q, s = quantize_float_array(arr, input_sq_mean=None if activation_sq_means is None else activation_sq_means.get(name))
        if s.ndim == 2:
            qmeta[name] = {"scheme": "per_row_group", "axis": 0, "group_size": INT8_PER_ROW_GROUP_SIZE}
        elif s.ndim > 0:
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int5_group64_actdiagclip_tiedemb_fp16_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q in quant_obj["quantized"].items():
        ...
        if qmeta.get(name, {}).get("scheme") == "per_row_group":
        ...
        elif qmeta.get(name, {}).get("scheme") == "per_row" or scale.ndim > 0:
...
def main() -> None:
    ...
    activation_sq_means = collect_activation_sq_means(model, args, log_fn=log)
    quant_obj, quant_stats = quantize_state_dict_int8(flat_state, activation_sq_means)
