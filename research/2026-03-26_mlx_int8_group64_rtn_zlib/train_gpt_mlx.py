...
# ==============================================================================
# - per-row-group int8 for 2D float tensors
...
def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        # Matrices keep the same row-wise structure as baseline, but use smaller
        # column groups so a single outlier block does not set the scale for the
        # entire row.
        rows, cols = f32.shape
        if cols > INT8_PER_ROW_GROUP_SIZE:
            ...
            for group_idx in range(num_groups):
                ...
                group_scale = np.maximum(clip_abs / 127.0, 1.0 / 127.0).astype(np.float32, copy=False)
                q[:, start:end] = np.clip(np.round(clipped / group_scale[:, None]), -127, 127).astype(np.int8, copy=False)
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int8_clean_per_row_group64_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
