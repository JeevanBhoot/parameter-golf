...
# - 5-bit packed storage for INT5 tensors
...
def pack_int5_values(q: np.ndarray) -> np.ndarray:
    q_flat = np.asarray(q, dtype=np.int8).reshape(-1)
    if q_flat.size == 0:
        return np.empty((0,), dtype=np.uint8)
    q_u5 = (q_flat.astype(np.int16) & 0x1F).astype(np.uint64, copy=False)
    pad = (-q_u5.size) % 8
    if pad:
        q_u5 = np.pad(q_u5, (0, pad), constant_values=0)
    q_words = q_u5.reshape(-1, 8)
    packed = (
        q_words[:, 0]
        | (q_words[:, 1] << 5)
        | (q_words[:, 2] << 10)
    ...
    out = np.empty((packed.size, 5), dtype=np.uint8)
    out[:, 0] = (packed & 0xFF).astype(np.uint8, copy=False)
    out[:, 1] = ((packed >> 8) & 0xFF).astype(np.uint8, copy=False)
    out[:, 2] = ((packed >> 16) & 0xFF).astype(np.uint8, copy=False)
    out[:, 3] = ((packed >> 24) & 0xFF).astype(np.uint8, copy=False)
    out[:, 4] = ((packed >> 32) & 0xFF).astype(np.uint8, copy=False)
    return np.ascontiguousarray(out.reshape(-1))


def unpack_int5_values(packed: np.ndarray, shape: tuple[int, ...], numel: int) -> np.ndarray:
    packed_u8 = np.asarray(packed, dtype=np.uint8).reshape(-1)
    if numel == 0:
        return np.empty(shape, dtype=np.int8)
    packed_words = packed_u8.reshape(-1, 5).astype(np.uint64, copy=False)
    merged = (
        packed_words[:, 0]
        | (packed_words[:, 1] << 8)
        | (packed_words[:, 2] << 16)
    ...
    q_u5 = np.empty((merged.size, 8), dtype=np.uint8)
    q_u5[:, 0] = (merged & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 1] = ((merged >> 5) & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 2] = ((merged >> 10) & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 3] = ((merged >> 15) & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 4] = ((merged >> 20) & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 5] = ((merged >> 25) & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 6] = ((merged >> 30) & 0x1F).astype(np.uint8, copy=False)
    q_u5[:, 7] = ((merged >> 35) & 0x1F).astype(np.uint8, copy=False)
    q_i16 = q_u5.reshape(-1)[:numel].astype(np.int16, copy=False)
    q_i16[q_i16 >= 16] -= 32
    return np.ascontiguousarray(q_i16.astype(np.int8, copy=False).reshape(shape))
        ...
        if cols > INT8_PER_ROW_GROUP_SIZE:
            ...
            for group_idx in range(num_groups):
                ...
                group_scale = np.maximum(
                    clip_abs / float(INT8_RTN_QMAX),
                    1.0 / float(INT8_RTN_QMAX),
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    for name, arr in flat_state.items():
        ...
        packed_q = pack_int5_values(q)
        qinfo: dict[str, object] = {
            "value_format": "int5_packed",
            "shape": tuple(int(dim) for dim in q.shape),
            "numel": int(q.size),
        }
        if s.ndim == 2:
            qinfo.update({"scheme": "per_row_group", "axis": 0, "group_size": INT8_PER_ROW_GROUP_SIZE})
        elif s.ndim > 0:
            qinfo.update({"scheme": "per_row", "axis": 0})
        qmeta[name] = qinfo
        quantized[name] = packed_q
        scales[name] = s
        dtypes[name] = str(arr.dtype).split(".")[-1]
        stats["int8_payload_bytes"] += int(packed_q.nbytes + s.nbytes)
    obj: dict[str, object] = {
        "__quant_format__": "int5_group64_tiedemb_fp16_packbits_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q in quant_obj["quantized"].items():
        qinfo = qmeta.get(name, {})
        if qinfo.get("value_format") == "int5_packed":
            q_np = unpack_int5_values(
                q,
                tuple(int(dim) for dim in qinfo["shape"]),
                int(qinfo["numel"]),
            )
        else:
        ...
        if qinfo.get("scheme") == "per_row_group":
        ...
        elif qinfo.get("scheme") == "per_row" or scale.ndim > 0:
