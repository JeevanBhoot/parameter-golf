...
# - higher-bit tied head quantization for the embedding/output matrix
...
def quantize_float_array(arr: mx.array, qmax: int = INT8_RTN_QMAX) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    for name, arr in flat_state.items():
        ...
        q, s = quantize_float_array(arr, qmax=qmax)
        if s.ndim == 2:
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int6_group64_headint8_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
