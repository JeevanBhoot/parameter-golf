...
def _mse_search_quantize_per_row(f32: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    q_candidates = np.clip(np.round(clipped / scale[:, None, :]), -127, 127).astype(np.int8, copy=False)
...
def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        # Search a small set of clip quantiles per row and keep the candidate
        # with the lowest dequantized weight MSE.
        return _mse_search_quantize_per_row(f32)

    # Vectors / scalars use the same MSE clip search, but with one scale per tensor.
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    for name, arr in flat_state.items():
        ...
        if int(arr.size) <= INT8_KEEP_FLOAT_MAX_NUMEL or name == "blocks.8.mlp.proj.weight":
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int8_mseclipsearch_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
