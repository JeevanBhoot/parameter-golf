...
# - 6-bit packed storage for INT6 tensors
...
def pack_int6_values(q: np.ndarray) -> np.ndarray:
    q_flat = np.asarray(q, dtype=np.int8).reshape(-1)
    if q_flat.size == 0:
        return np.empty((0,), dtype=np.uint8)
    q_u6 = (q_flat.astype(np.int16) & 0x3F).astype(np.uint8, copy=False)
    pad = (-q_u6.size) % 4
    if pad:
        q_u6 = np.pad(q_u6, (0, pad))
    q_words = q_u6.reshape(-1, 4).astype(np.uint32, copy=False)
    packed = (
        q_words[:, 0]
        | (q_words[:, 1] << 6)
        | (q_words[:, 2] << 12)
...
def unpack_int6_values(packed: np.ndarray, shape: tuple[int, ...], numel: int) -> np.ndarray:
    packed_u8 = np.asarray(packed, dtype=np.uint8).reshape(-1)
    if numel == 0:
        return np.empty(shape, dtype=np.int8)
    packed_words = packed_u8.reshape(-1, 3).astype(np.uint32, copy=False)
    merged = packed_words[:, 0] | (packed_words[:, 1] << 8) | (packed_words[:, 2] << 16)
    q_u6 = np.empty((merged.size, 4), dtype=np.uint8)
    q_u6[:, 0] = (merged & 0x3F).astype(np.uint8, copy=False)
    q_u6[:, 1] = ((merged >> 6) & 0x3F).astype(np.uint8, copy=False)
    q_u6[:, 2] = ((merged >> 12) & 0x3F).astype(np.uint8, copy=False)
    q_u6[:, 3] = ((merged >> 18) & 0x3F).astype(np.uint8, copy=False)
    q_i16 = q_u6.reshape(-1)[:numel].astype(np.int16, copy=False)
    q_i16[q_i16 >= 32] -= 64
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    for name, arr in flat_state.items():
        ...
        packed_q = pack_int6_values(q)
        qinfo: dict[str, object] = {
            "value_format": "int6_packed",
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int6_group64_tiedemb_fp16_packbits_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q in quant_obj["quantized"].items():
        ...
        if qinfo.get("value_format") == "int6_packed":
            q_np = unpack_int6_values(
                q,
