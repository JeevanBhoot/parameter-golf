...
def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    ...
    q = np.clip(np.round(np.clip(f32, -clip_abs, clip_abs) / scale), -INT8_RTN_QMAX, INT8_RTN_QMAX).astype(np.int8, copy=False)
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int6_rtn_packbits_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q in quant_obj["quantized"].items():
        ...
        if qinfo.get("scheme") == "per_row" or scale.ndim > 0:
