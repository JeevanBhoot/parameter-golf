...
INT8_RTN_QMAX = 15
INT8_AFFINE_QMAX = 2 * INT8_RTN_QMAX
...
def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray, np.ndarray | None, str | None]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        # Matrices use per-row group affine quantization so each local chunk gets
        # its own offset and scale instead of assuming symmetry around zero.
        rows, cols = f32.shape
        if cols > INT8_PER_ROW_GROUP_SIZE:
            ...
            offset = np.empty((rows, num_groups), dtype=np.float32)
            for group_idx in range(num_groups):
                start = group_idx * INT8_PER_ROW_GROUP_SIZE
                end = min(start + INT8_PER_ROW_GROUP_SIZE, cols)
                chunk = f32[:, start:end]
                if chunk.size:
                    group_min = chunk.min(axis=1).astype(np.float32, copy=False)
                    group_max = chunk.max(axis=1).astype(np.float32, copy=False)
                else:
                    group_min = np.empty((rows,), dtype=np.float32)
                    group_max = np.empty((rows,), dtype=np.float32)
                group_scale = np.maximum(
                    (group_max - group_min) / float(INT8_AFFINE_QMAX),
                    1e-8,
                ).astype(np.float32, copy=False)
                q[:, start:end] = np.clip(
                    np.round((chunk - group_min[:, None]) / group_scale[:, None]),
                    0,
                    INT8_AFFINE_QMAX,
                ).astype(np.int8, copy=False)
                scale[:, group_idx] = group_scale
                offset[:, group_idx] = group_min
            return (
                np.ascontiguousarray(q),
                np.ascontiguousarray(scale.astype(INT8_PER_ROW_SCALE_DTYPE, copy=False)),
                np.ascontiguousarray(offset.astype(INT8_PER_ROW_OFFSET_DTYPE, copy=False)),
                "per_row_group_affine",
            )

        row_min = f32.min(axis=1).astype(np.float32, copy=False) if f32.size else np.empty((rows,), dtype=np.float32)
        row_max = f32.max(axis=1).astype(np.float32, copy=False) if f32.size else np.empty((rows,), dtype=np.float32)
        scale = np.maximum((row_max - row_min) / float(INT8_AFFINE_QMAX), 1e-8).astype(np.float32, copy=False)
        q = np.clip(
            np.round((f32 - row_min[:, None]) / scale[:, None]),
            0,
            INT8_AFFINE_QMAX,
        ).astype(np.int8, copy=False)
        return (
            np.ascontiguousarray(q),
            np.ascontiguousarray(scale.astype(INT8_PER_ROW_SCALE_DTYPE, copy=False)),
            np.ascontiguousarray(row_min.astype(INT8_PER_ROW_OFFSET_DTYPE, copy=False)),
            "per_row_affine",
        )
    ...
    return np.ascontiguousarray(q), scale, None, None
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    for name, arr in flat_state.items():
        ...
        q, s, o, scheme = quantize_float_array(arr)
        if scheme == "per_row_group_affine":
            qmeta[name] = {"scheme": scheme, "axis": 0, "group_size": INT8_PER_ROW_GROUP_SIZE}
            offsets[name] = o
        elif scheme == "per_row_affine":
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int5_group64_affine_tiedemb_fp16_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
    ...
    for name, q in quant_obj["quantized"].items():
        ...
        if scheme == "per_row_group_affine":
            ...
            for group_idx in range(scale.shape[1]):
                ...
                out_arr[:, start:end] = (
                    q_np[:, start:end].astype(np.float32) * scale[:, group_idx][:, None]
                    + offset[:, group_idx][:, None]
                )
        elif scheme == "per_row_affine":
            offset = np.asarray(offsets[name], dtype=np.float32)
            out_arr = (
                q_np.astype(np.float32) * scale.reshape((q_np.shape[0],) + (1,) * (q_np.ndim - 1))
                + offset.reshape((q_np.shape[0],) + (1,) * (q_np.ndim - 1))
            )
        elif scheme == "per_row_group":
            group_size = int(qmeta[name]["group_size"])
            out_arr = np.empty(q_np.shape, dtype=np.float32)
            for group_idx in range(scale.shape[1]):
                start = group_idx * group_size
